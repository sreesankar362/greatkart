...
# if user is not authenticated
        ...
        try:
            cart = CartModel.objects.get(cart_id=_cart_id(request))  # get cart using the cart id
            # present in the session
        except CartModel.DoesNotExist:
            cart = CartModel.objects.create(cart_id=_cart_id(request))
        cart.save()
...
class Cart(View):
    ...
    def get(self, request,grand_total=0, total=0,tax=0, quantity=0,cart_items=None, *args, **kwargs):
        try:
            if request.user.is_authenticated:
